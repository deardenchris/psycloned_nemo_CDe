MODULE iceistate
  ...
  SUBROUTINE ice_istate
    ...
    IF (lwp) WRITE(numout, FMT = *) 'ice_istate: sea-ice initialization '
    IF (lwp) WRITE(numout, FMT = *) '~~~~~~~~~~'
    DO jl = 1, jpl
      t_su(:, :, jl) = rt0 * tmask(:, :, 1)
      cnd_ice(:, :, jl) = 0._wp
                  ...
                  IF (jl0 /= jpl) THEN
                    DO jl = jpl, jl0 + 1, - 1
                      IF (za_i_ini(ji, jj, jl) > za_i_ini(ji, jj, jl - 1)) THEN
                        ...
                        za_i_ini(ji, jj, 1 : jl - 1) = za_i_ini(ji, jj, 1 : jl - 1) + zdv / MAX(REAL(jl - 1) * zht_i_ini(ji, jj), &
&epsi10)
                      END IF
                    END DO
                  END IF
              ...
              IF (lwp .AND. SUM(itest) /= 4) THEN
                WRITE(numout, FMT = *)
                WRITE(numout, FMT = *) ' !!!! ALERT itest is not equal to 4      !!! '
                WRITE(numout, FMT = *) ' !!!! Something is wrong in the SI3 initialization procedure '
                WRITE(numout, FMT = *)
                WRITE(numout, FMT = *) ' *** itest_i (i=1,4) = ', itest(:)
                WRITE(numout, FMT = *) ' zat_i_ini : ', zat_i_ini(ji, jj)
                WRITE(numout, FMT = *) ' zht_i_ini : ', zht_i_ini(ji, jj)
              END IF
            END IF
          END DO
        END DO
      ...
      IF (nn_icesal /= 2) THEN
        ...
        sv_i = s_i * v_i
      END IF
      !$OMP parallel default(shared), private(ji,jj,jk,jl,ztmelts)
        ...
        DO jl = 1, jpl
          DO jj = 1, jpj
            DO ji = 1, jpi
              ...
              e_i(ji, jj, jk, jl) = zswitch(ji, jj) * rhoi * (rcpi * (ztmelts - t_i(ji, jj, jk, jl)) + rLfus * (1._wp - (ztmelts - &
&rt0) / MIN((t_i(ji, jj, jk, jl) - rt0), - epsi20)) - rcp * (ztmelts - rt0))
  ...
  SUBROUTINE ice_istate_init
    ...
    NAMELIST /namini/ ln_iceini, ln_iceini_file, rn_thres_sst, rn_hts_ini_n, rn_hts_ini_s, rn_hti_ini_n, rn_hti_ini_s, &
&rn_ati_ini_n, rn_ati_ini_s, rn_smi_ini_n, rn_smi_ini_s, rn_tmi_ini_n, rn_tmi_ini_s, sn_hti, sn_hts, sn_ati, sn_tsu, sn_tmi, &
&sn_smi, cn_dir
...
902 IF (ios > 0) CALL ctl_nam(ios, 'namini in configuration namelist', lwp)
    ...
    IF (lwp) THEN
      WRITE(numout, FMT = *)
      WRITE(numout, FMT = *) 'ice_istate_init: ice parameters inititialisation '
      WRITE(numout, FMT = *) '~~~~~~~~~~~~~~~'
      WRITE(numout, FMT = *) '   Namelist namini:'
      WRITE(numout, FMT = *) '      initialization with ice (T) or not (F)                 ln_iceini       = ', ln_iceini
      WRITE(numout, FMT = *) '      ice initialization from a netcdf file                  ln_iceini_file  = ', ln_iceini_file
      WRITE(numout, FMT = *) '      max delta ocean temp. above Tfreeze with initial ice   rn_thres_sst    = ', rn_thres_sst
      WRITE(numout, FMT = *) '      initial snow thickness in the north                    rn_hts_ini_n    = ', rn_hts_ini_n
      WRITE(numout, FMT = *) '      initial snow thickness in the south                    rn_hts_ini_s    = ', rn_hts_ini_s
      WRITE(numout, FMT = *) '      initial ice thickness  in the north                    rn_hti_ini_n    = ', rn_hti_ini_n
      WRITE(numout, FMT = *) '      initial ice thickness  in the south                    rn_hti_ini_s    = ', rn_hti_ini_s
      WRITE(numout, FMT = *) '      initial ice concentr.  in the north                    rn_ati_ini_n    = ', rn_ati_ini_n
      WRITE(numout, FMT = *) '      initial ice concentr.  in the north                    rn_ati_ini_s    = ', rn_ati_ini_s
      WRITE(numout, FMT = *) '      initial  ice salinity  in the north                    rn_smi_ini_n    = ', rn_smi_ini_n
      WRITE(numout, FMT = *) '      initial  ice salinity  in the south                    rn_smi_ini_s    = ', rn_smi_ini_s
      WRITE(numout, FMT = *) '      initial  ice/snw temp  in the north                    rn_tmi_ini_n    = ', rn_tmi_ini_n
      WRITE(numout, FMT = *) '      initial  ice/snw temp  in the south                    rn_tmi_ini_s    = ', rn_tmi_ini_s
    END IF
    IF (ln_iceini_file) THEN
