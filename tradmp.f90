...
902 IF (ios > 0) CALL ctl_nam(ios, 'namtra_dmp in configuration namelist', lwp)
    ...
    IF (ln_tradmp) THEN
      ...
      SELECT CASE (nn_zdmp)
      CASE (0)
        IF (lwp) WRITE(numout, *) '   tracer damping as specified by mask'
      CASE (1)
        IF (lwp) WRITE(numout, *) '   no tracer damping in the mixing layer (kz > 5 cm2/s)'
      CASE (2)
