MODULE diawri
  ...
  INTEGER FUNCTION dia_wri_alloc()
    ...
    ierr = 0
    ALLOCATE(ndex_hT(jpi * jpj), ndex_T(jpi * jpj * jpk), ndex_hU(jpi * jpj), ndex_U(jpi * jpj * jpk), ndex_hV(jpi * jpj), &
&ndex_V(jpi * jpj * jpk), STAT = ierr(1))
  ...
  SUBROUTINE dia_wri(kt)
    ...
    IF (kt == nit000) THEN
      ...
      IF (lwp) WRITE(numout, FMT = *) 'Date 0 used :', nit000, ' YEAR ', nyear, ' MONTH ', nmonth, ' DAY ', nday, 'Julian day : ', &
&zjulian
      ...
      CALL histbeg(clhstnam, jpi, glamt, jpj, gphit, iimi, iima - iimi + 1, ijmi, ijma - ijmi + 1, nit000 - 1, zjulian, rdt, nh_T, &
&nid_T, domain_id = nidom, snc4chunks = snc4set)
      ...
      IF (lwp) WRITE(numout, FMT = *) " Name of NETCDF file ", clhstnam
      CALL histbeg(clhstnam, jpi, glamu, jpj, gphiu, iimi, iima - iimi + 1, ijmi, ijma - ijmi + 1, nit000 - 1, zjulian, rdt, nh_U, &
&nid_U, domain_id = nidom, snc4chunks = snc4set)
      ...
      IF (lwp) WRITE(numout, FMT = *) " Name of NETCDF file ", clhstnam
      CALL histbeg(clhstnam, jpi, glamv, jpj, gphiv, iimi, iima - iimi + 1, ijmi, ijma - ijmi + 1, nit000 - 1, zjulian, rdt, nh_V, &
&nid_V, domain_id = nidom, snc4chunks = snc4set)
      ...
      IF (lwp) WRITE(numout, FMT = *) " Name of NETCDF file ", clhstnam
      CALL histbeg(clhstnam, jpi, glamt, jpj, gphit, iimi, iima - iimi + 1, ijmi, ijma - ijmi + 1, nit000 - 1, zjulian, rdt, nh_W, &
&nid_W, domain_id = nidom, snc4chunks = snc4set)
      ...
      IF (ln_linssh) THEN
        CALL histdef(nid_T, "sosst_cd", "Concentration/Dilution term on temperature", "KgC/m2/s", jpi, jpj, nh_T, 1, 1, 1, - 99, &
&32, clop, zsto, zout)
        CALL histdef(nid_T, "sosss_cd", "Concentration/Dilution term on salinity", "KgPSU/m2/s", jpi, jpj, nh_T, 1, 1, 1, - 99, &
&32, clop, zsto, zout)
      END IF
      ...
      IF (ln_icebergs) THEN
        ...
        CALL histdef(nid_T, "berg_floating_melt", "Melt rate of icebergs + bits", "kg/m2/s", jpi, jpj, nh_T, 1, 1, 1, - 99, 32, &
&clop, zsto, zout)
        CALL histdef(nid_T, "berg_stored_ice", "Accumulated ice mass by class", "kg", jpi, jpj, nh_T, nclasses, 1, nclasses, nb_T, &
&32, clop, zsto, zout)
        IF (ln_bergdia) THEN
          CALL histdef(nid_T, "berg_melt", "Melt rate of icebergs", "kg/m2/s", jpi, jpj, nh_T, 1, 1, 1, - 99, 32, clop, zsto, zout)
          CALL histdef(nid_T, "berg_buoy_melt", "Buoyancy component of iceberg melt rate", "kg/m2/s", jpi, jpj, nh_T, 1, 1, 1, &
&- 99, 32, clop, zsto, zout)
          CALL histdef(nid_T, "berg_eros_melt", "Erosion component of iceberg melt rate", "kg/m2/s", jpi, jpj, nh_T, 1, 1, 1, &
&- 99, 32, clop, zsto, zout)
          CALL histdef(nid_T, "berg_conv_melt", "Convective component of iceberg melt rate", "kg/m2/s", jpi, jpj, nh_T, 1, 1, 1, &
&- 99, 32, clop, zsto, zout)
          CALL histdef(nid_T, "berg_virtual_area", "Virtual coverage by icebergs", "m2", jpi, jpj, nh_T, 1, 1, 1, - 99, 32, clop, &
&zsto, zout)
          CALL histdef(nid_T, "bits_src", "Mass source of bergy bits", "kg/m2/s", jpi, jpj, nh_T, 1, 1, 1, - 99, 32, clop, zsto, &
&zout)
          CALL histdef(nid_T, "bits_melt", "Melt rate of bergy bits", "kg/m2/s", jpi, jpj, nh_T, 1, 1, 1, - 99, 32, clop, zsto, &
&zout)
          ...
          CALL histdef(nid_T, "berg_real_calving", "Calving into iceberg class", "kg/s", jpi, jpj, nh_T, nclasses, 1, nclasses, &
&nb_T, 32, clop, zsto, zout)
        END IF
      END IF
      IF (.NOT. ln_cpl) THEN
        CALL histdef(nid_T, "sohefldp", "Surface Heat Flux: Damping", "W/m2", jpi, jpj, nh_T, 1, 1, 1, - 99, 32, clop, zsto, zout)
        CALL histdef(nid_T, "sowafldp", "Surface Water Flux: Damping", "Kg/m2/s", jpi, jpj, nh_T, 1, 1, 1, - 99, 32, clop, zsto, &
&zout)
        CALL histdef(nid_T, "sosafldp", "Surface salt flux: damping", "Kg/m2/s", jpi, jpj, nh_T, 1, 1, 1, - 99, 32, clop, zsto, &
&zout)
      END IF
...
&zout)
      ...
      IF (ln_wave .AND. ln_sdw) THEN
        CALL histdef(nid_U, "sdzocrtx", "Stokes Drift Zonal Current", "m/s", jpi, jpj, nh_U, ipk, 1, ipk, nz_U, 32, clop, zsto, &
&zout)
      END IF
      ...
      IF (ln_wave .AND. ln_sdw) THEN
        CALL histdef(nid_V, "sdmecrty", "Stokes Drift Meridional Current", "m/s", jpi, jpj, nh_V, ipk, 1, ipk, nz_V, 32, clop, &
&zsto, zout)
      END IF
      ...
      IF (ln_zdfddm) THEN
        CALL histdef(nid_W, "voddmavs", "Salt Vertical Eddy Diffusivity", "m2/s", jpi, jpj, nh_W, ipk, 1, ipk, nz_W, 32, clop, &
&zsto, zout)
      END IF
      IF (ln_wave .AND. ln_sdw) THEN
        CALL histdef(nid_W, "sdvecrtz", "Stokes Drift Vertical Current", "m/s", jpi, jpj, nh_W, ipk, 1, ipk, nz_W, 32, clop, zsto, &
&zout)
      END IF
  ...
  SUBROUTINE dia_wri_state(cdfile_name)
    ...
    IF (lwp) WRITE(numout, FMT = *) '                and named :', cdfile_name, '...nc'
