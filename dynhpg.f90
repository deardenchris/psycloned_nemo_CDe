MODULE dynhpg
  ...
  SUBROUTINE dyn_hpg(kt)
    ...
    END IF
    CALL profile_psy_data3 % PreStart('dyn_hpg', 'r3', 0, 0)
...
902 IF (ios > 0) CALL ctl_nam(ios, 'namdyn_hpg in configuration namelist', lwp)
    ...
    IF (lwp) THEN
      ...
      SELECT CASE (nhpg)
      CASE (np_zco)
        WRITE(numout, *) '   ==>>>   z-coord. - full steps '
      CASE (np_zps)
        WRITE(numout, *) '   ==>>>   z-coord. - partial steps (interpolation)'
      CASE (np_sco)
        WRITE(numout, *) '   ==>>>   s-coord. (standard jacobian formulation)'
      CASE (np_djc)
        WRITE(numout, *) '   ==>>>   s-coord. (Density Jacobian: Cubic polynomial)'
      CASE (np_prj)
        WRITE(numout, *) '   ==>>>   s-coord. (Pressure Jacobian: Cubic polynomial)'
      CASE (np_isf)
        WRITE(numout, *) '   ==>>>   s-coord. (standard jacobian formulation) for isf'
      END SELECT
      WRITE(numout, *)
    END IF
    ...
    ELSE
      ...
      DO jj = 1, jpj
        DO ji = 1, jpi
          ...
          END DO
          IF (ikt >= 2) ziceload(ji, jj) = ziceload(ji, jj) + (2._wp * znad + zrhdtop_isf(ji, jj) + zrhd(ji, jj, ikt - 1)) * (risfdep(ji, jj) - gdept_n(ji, jj, ikt - 1))
        END DO
      END DO
      riceload(:, :) = ziceload(:, :)
      !$ACC END KERNELS
  ...
  SUBROUTINE hpg_sco(kt)
    ...
    IF (ln_wd_il) THEN
      ...
      END DO
      !$ACC END KERNELS
      CALL profile_psy_data1 % PreStart('hpg_sco', 'r1', 0, 0)
  ...
  SUBROUTINE hpg_isf(kt)
    ...
    END DO
    !$ACC END KERNELS
    CALL profile_psy_data0 % PreStart('hpg_isf', 'r0', 0, 0)
  ...
  SUBROUTINE hpg_prj(kt)
    ...
    END DO
    !$ACC END KERNELS
    CALL profile_psy_data5 % PreStart('hpg_prj', 'r5', 0, 0)
    DO jk = 1, jpkm1
      DO jj = 2, jpjm1
        DO ji = 2, jpim1
          zpwes = 0._wp
  ...
  SUBROUTINE cspline(fsp, xsp, asp, bsp, csp, dsp, polynomial_type)
    ...
    ELSE IF (polynomial_type == 2) THEN
      ...
      END DO
    ELSE
      CALL profile_psy_data1 % PreStart('cspline', 'r1', 0, 0)
