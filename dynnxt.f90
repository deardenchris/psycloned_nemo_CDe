MODULE dynnxt
  ...
  SUBROUTINE dyn_nxt(kt)
    ...
    IF (ln_dynspg_ts) THEN
      ...
      ! !$OMP parallel default(shared), private(jk,zue,zve) ! CDe causes runtime
    ...
    ELSE
      IF (ln_linssh) THEN
        !$OMP parallel default(shared), private(ji,jj,jk,zuf,zvf)
        !$OMP do schedule(static)
        DO jk = 1, jpkm1
          DO jj = 1, jpj
            DO ji = 1, jpi
...
&r1_hisf_tbl(ji, jj)) * ralpha(ji, jj) * tmask(ji, jj, jk)
        ...
        ELSE
          ...
          !$OMP parallel default(shared), private(ji,jj,jk,zue3a,zue3b,zue3n,zuf,zve3a,zve3b,zve3n,zvf)
          !$OMP do schedule(static)
          DO jk = 1, jpkm1
            DO jj = 1, jpj
              DO ji = 1, jpi
      ...
      IF (ln_dynspg_ts .AND. ln_bt_fw) THEN
        ...
        ! !$OMP parallel default(shared), private(jk) ! CDe causes
        ! runtime crash due to race condition
        ! !$OMP do schedule(static)
        DO jk = 2, jpkm1
