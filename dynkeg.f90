        ...
        DO ib_bdy = 1, nb_bdy
          IF (cn_dyn3d(ib_bdy) /= 'none') THEN
            ...
            DO jb = 1, idx_bdy(ib_bdy) % nblenrim(igrd)
              ...
              DO jk = 1, jpkm1
                zhke(ji, jj, jk) = 0._wp
                zweightu = 8._wp * (umask(ji - 1, jj, jk) + umask(ji, jj, jk)) + 2._wp * (umask(ji - 1, jj - 1, jk) + umask(ji - 1, jj + 1, jk) + umask(ji, jj - 1, jk) + umask(ji, jj + 1, jk))
                zweightv = 8._wp * (vmask(ji, jj - 1, jk) + vmask(ji, jj - 1, jk)) + 2._wp * (vmask(ji - 1, jj - 1, jk) + vmask(ji + 1, jj - 1, jk) + vmask(ji - 1, jj, jk) + vmask(ji + 1, jj, jk))
                zu = 8._wp * (un(ji - 1, jj, jk) * un(ji - 1, jj, jk) + un(ji, jj, jk) * un(ji, jj, jk)) + (un(ji - 1, jj - 1, jk) + un(ji - 1, jj + 1, jk)) * (un(ji - 1, jj - 1, jk) + un(ji - 1, jj + 1, jk)) + (un(ji, jj - 1, jk) + un(ji, jj + 1, jk)) * (un(ji, jj - 1, jk) + un(ji, jj + 1, jk))
                zv = 8._wp * (vn(ji, jj - 1, jk) * vn(ji, jj - 1, jk) + vn(ji, jj, jk) * vn(ji, jj, jk)) + (vn(ji - 1, jj - 1, jk) + vn(ji + 1, jj - 1, jk)) * (vn(ji - 1, jj - 1, jk) + vn(ji + 1, jj - 1, jk)) + (vn(ji - 1, jj, jk) + vn(ji + 1, jj, jk)) * (vn(ji - 1, jj, jk) + vn(ji + 1, jj, jk))
                IF (zweightu > 0._wp) zhke(ji, jj, jk) = zhke(ji, jj, jk) + zu / (2._wp * zweightu)
                IF (zweightv > 0._wp) zhke(ji, jj, jk) = zhke(ji, jj, jk) + zv / (2._wp * zweightv)
