MODULE obs_prep
  ...
  SUBROUTINE obs_pre_surf(surfdata, surfdataqc, ld_nea, ld_bound_reject, kqc_cutoff)
    ...
    !$ACC END KERNELS
    CALL profile_psy_data1 % PreStart('obs_pre_surf', 'r1', 0, 0)
    CALL obs_surf_compress(surfdata, surfdataqc, .TRUE., numout, lvalid = llvalid)
    DEALLOCATE(llvalid)
    IF (lwp) THEN
      WRITE(numout, *)
      WRITE(numout, *) ' ' // surfdataqc % cvars(1) // ' data outside time domain                  = ', iotdobsmpp
      WRITE(numout, *) ' Remaining ' // surfdataqc % cvars(1) // ' data that failed grid search    = ', igrdobsmpp
      WRITE(numout, *) ' Remaining ' // surfdataqc % cvars(1) // ' data outside space domain       = ', iosdsobsmpp
      WRITE(numout, *) ' Remaining ' // surfdataqc % cvars(1) // ' data at land points             = ', ilansobsmpp
      IF (ld_nea) THEN
        WRITE(numout, *) ' Remaining ' // surfdataqc % cvars(1) // ' data near land points (removed) = ', inlasobsmpp
      ELSE
        WRITE(numout, *) ' Remaining ' // surfdataqc % cvars(1) // ' data near land points (kept)    = ', inlasobsmpp
      END IF
      WRITE(numout, *) ' Remaining ' // surfdataqc % cvars(1) // ' data near open boundary (removed) = ', ibdysobsmpp
      WRITE(numout, *) ' ' // surfdataqc % cvars(1) // ' data accepted                             = ', surfdataqc % nsurfmpp
      WRITE(numout, *)
    ...
    END DO
    !$ACC END KERNELS
    CALL profile_psy_data2 % PreStart('obs_pre_surf', 'r2', 0, 0)
...
1999 FORMAT(10X, I9, 5X, I17)
  ...
  SUBROUTINE obs_pre_prof(profdata, prodatqc, ld_var1, ld_var2, kpi, kpj, kpk, zmask1, pglam1, pgphi1, zmask2, pglam2, pgphi2, ld_nea, ld_bound_reject, kdailyavtypes, kqc_cutoff)
    ...
    END DO
    !$ACC END KERNELS
    CALL profile_psy_data1 % PreStart('obs_pre_prof', 'r1', 0, 0)
    ...
    IF (lwp) THEN
      WRITE(numout, *)
      WRITE(numout, *) ' Profiles outside time domain                     = ', iotdobsmpp
      WRITE(numout, *) ' Remaining profiles that failed grid search       = ', igrdobsmpp
      WRITE(numout, *) ' Remaining ' // prodatqc % cvars(1) // ' data outside space domain       = ', iosdv1obsmpp
      WRITE(numout, *) ' Remaining ' // prodatqc % cvars(1) // ' data at land points             = ', ilanv1obsmpp
      IF (ld_nea) THEN
        WRITE(numout, *) ' Remaining ' // prodatqc % cvars(1) // ' data near land points (removed) = ', inlav1obsmpp
      ELSE
        WRITE(numout, *) ' Remaining ' // prodatqc % cvars(1) // ' data near land points (kept)    = ', inlav1obsmpp
      END IF
      IF (TRIM(profdata % cvars(1)) == 'UVEL') THEN
        WRITE(numout, *) ' U observation rejected since V rejected     = ', iuvchku
      END IF
      WRITE(numout, *) ' Remaining ' // prodatqc % cvars(1) // ' data near open boundary (removed) = ', ibdyv1obsmpp
      WRITE(numout, *) ' ' // prodatqc % cvars(1) // ' data accepted                             = ', prodatqc % nvprotmpp(1)
      WRITE(numout, *) ' Remaining ' // prodatqc % cvars(2) // ' data outside space domain       = ', iosdv2obsmpp
      WRITE(numout, *) ' Remaining ' // prodatqc % cvars(2) // ' data at land points             = ', ilanv2obsmpp
      IF (ld_nea) THEN
        WRITE(numout, *) ' Remaining ' // prodatqc % cvars(2) // ' data near land points (removed) = ', inlav2obsmpp
      ELSE
        WRITE(numout, *) ' Remaining ' // prodatqc % cvars(2) // ' data near land points (kept)    = ', inlav2obsmpp
      END IF
      IF (TRIM(profdata % cvars(1)) == 'UVEL') THEN
        WRITE(numout, *) ' V observation rejected since U rejected     = ', iuvchkv
      END IF
      WRITE(numout, *) ' Remaining ' // prodatqc % cvars(2) // ' data near open boundary (removed) = ', ibdyv2obsmpp
      WRITE(numout, *) ' ' // prodatqc % cvars(2) // ' data accepted                             = ', prodatqc % nvprotmpp(2)
      WRITE(numout, *)
      WRITE(numout, *) ' Number of observations per time step :'
      WRITE(numout, *)
      WRITE(numout, '(10X,A,5X,A,5X,A,A)') 'Time step', 'Profiles', '     ' // prodatqc % cvars(1) // '     ', '     ' // prodatqc % cvars(2) // '     '
      WRITE(numout, 998)
    END IF
    CALL profile_psy_data1 % PostEnd
    !$ACC KERNELS
    ...
    END DO
    !$ACC END KERNELS
    CALL profile_psy_data2 % PreStart('obs_pre_prof', 'r2', 0, 0)
...
999 FORMAT(10X, I9, 5X, I8, 5X, I11, 5X, I8)
  ...
  SUBROUTINE calc_month_len(iyear, imonth_len)
    ...
    TYPE(profile_PSyDataType), TARGET, SAVE :: profile_psy_data0
    IF (nleapy < 2) THEN
      CALL profile_psy_data0 % PreStart('calc_month_len', 'r0', 0, 0)
  ...
  SUBROUTINE obs_uv_rej(profdata, knumu, knumv, kqc_cutoff)
    ...
    DO jprof = 1, profdata % nprof
      IF ((profdata % npvsta(jprof, 1) /= profdata % npvsta(jprof, 2)) .OR. (profdata % npvend(jprof, 1) /= profdata % npvend(jprof, 2))) THEN
        ...
        RETURN
      END IF
      CALL profile_psy_data1 % PreStart('obs_uv_rej', 'r1', 0, 0)
