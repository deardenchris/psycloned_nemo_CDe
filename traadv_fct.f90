MODULE traadv_fct
  ...
  SUBROUTINE tra_adv_fct(kt, kit000, cdtype, p2dt, pun, pvn, pwn, ptb, ptn, pta, kjpt, kn_fct_h, kn_fct_v)
    ...
    zwi(:, :, :) = 0._wp
    DO jn = 1, kjpt
      !$OMP parallel default(shared), private(ji,jj,jk,zfm_ui,zfm_vj,zfm_wk,zfp_ui,zfp_vj,zfp_wk)
      ...
      CASE (41)
        ...
        !$OMP parallel default(shared), private(ji,jj,jk,zc2t_u,zc2t_v,zc4t_u,zc4t_v)
        !$OMP do schedule(static)
        DO jk = 1, jpkm1
          DO jj = 2, jpjm1
            DO ji = 2, jpim1
