MODULE icedyn
  ...
  SUBROUTINE ice_dyn(kt)
    ...
    CASE (np_dynADV2D)
      ...
      !$ACC END KERNELS
      CALL profile_psy_data4 % PreStart('ice_dyn', 'r4', 0, 0)
      CALL ice_dyn_adv(kt)
    ...
    IF (iom_use('icediv')) THEN
      ...
      CASE (np_dynADV1D, np_dynADV2D)
        ...
        DO jj = 2, jpjm1
          DO ji = 2, jpim1
            zdivu_i(ji, jj) = (e2u(ji, jj) * u_ice(ji, jj) - e2u(ji - 1, jj) * u_ice(ji - 1, jj) + e1v(ji, jj) * v_ice(ji, jj) - e1v(ji, jj - 1) * v_ice(ji, jj - 1)) * r1_e1e2t(ji, jj)
          END DO
        END DO
        !$ACC END KERNELS
        CALL profile_psy_data5 % PreStart('ice_dyn', 'r5', 0, 0)
      ...
      END SELECT
    END IF
    CALL profile_psy_data6 % PreStart('ice_dyn', 'r6', 0, 0)
  ...
  SUBROUTINE Hpiling
    ...
    TYPE(profile_PSyDataType), TARGET, SAVE :: profile_psy_data0
    TYPE(profile_PSyDataType), TARGET, SAVE :: profile_psy_data1
    CALL profile_psy_data0 % PreStart('hpiling', 'r0', 0, 0)
    ...
    END DO
    !$ACC END KERNELS
    CALL profile_psy_data1 % PreStart('hpiling', 'r1', 0, 0)
  ...
  SUBROUTINE ice_dyn_init
    INTEGER :: ios, ioptio
    NAMELIST /namdyn/ ln_dynALL, ln_dynRHGADV, ln_dynADV1D, ln_dynADV2D, rn_uice, rn_vice, rn_ishlat, ln_landfast_L16, ln_landfast_home, rn_depfra, rn_icebfr, rn_lfrelax, rn_tensile
