    ...
    DO jbdy = 1, nb_bdy
      ...
      CASE ('frs')
        ...
        CASE ('U')
          ...
          END DO
          !$ACC END KERNELS
          CALL profile_psy_data1 % PreStart('bdy_ice_dyn', 'r1', 0, 0)
        ...
        CASE ('V')
          ...
          END DO
          !$ACC END KERNELS
          CALL profile_psy_data2 % PreStart('bdy_ice_dyn', 'r2', 0, 0)
          CALL lbc_bdy_lnk('bdyice', v_ice(:, :), 'V', - 1., jbdy)
          CALL profile_psy_data2 % PostEnd
        END SELECT
      CASE DEFAULT
        CALL profile_psy_data3 % PreStart('bdy_ice_dyn', 'r3', 0, 0)
        CALL ctl_stop('bdy_ice_dyn : unrecognised option for open boundaries for ice fields')
        CALL profile_psy_data3 % PostEnd
      END SELECT
    END DO
    CALL profile_psy_data4 % PreStart('bdy_ice_dyn', 'r4', 0, 0)
