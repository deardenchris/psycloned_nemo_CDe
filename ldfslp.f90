MODULE ldfslp
  ...
  SUBROUTINE ldf_slp(kt, prd, pn2)
    ...
    ELSE
      ...
      END DO
    END IF
    !$OMP parallel default(shared), private(ji,jj,jk,zau,zav,zbu,zbv,zdepu,zdepv,zfi,zfj)
    !$OMP do schedule(static)
    DO jk = 2, jpkm1
      DO jj = 2, jpjm1
        DO ji = 2, jpim1
    ...
    !$OMP parallel default(shared), private(ji,jj,jk,zai,zaj,zbi,zbj,zbw,zci,zcj,zck,zfk)
    !$OMP do schedule(static)
    DO jk = 2, jpkm1
    ...
    !$OMP parallel default(shared), private(ji,jj,jk,zck,zcofw)
    !$OMP do schedule(static)
    DO jk = 2, jpkm1
      DO jj = 2, jpjm1, MAX(1, jpj - 3)
        DO ji = 2, jpim1
  ...
  SUBROUTINE ldf_slp_triad(kt)
    ...
    DO jl = 0, 1
      ip = jl
      jp = jl
      !$OMP parallel default(shared), private(ji,jj,jk,zdis,zdit,zdjs,zdjt,zdxrho_raw,zdyrho_raw)
      !$OMP do schedule(static)
      DO jk = 1, jpkm1
        DO jj = 1, jpjm1
          DO ji = 1, jpim1
    ...
    END DO
    DO kp = 0, 1
      ! !$OMP parallel default(shared), private(ji,jj,jk,zdks,zdkt,zdzrho_raw)
      ! !$OMP do schedule(static)
      DO jk = 1, jpkm1
        DO jj = 1, jpj
          DO ji = 1, jpi
    ...
    DO kp = 0, 1
      DO jl = 0, 1
        ip = jl
        jp = jl
        ! !$OMP parallel default(shared), private(ji,jj,jk,zbti,zbtj,zbu,zbv,ze3_e1,ze3_e2,zfacti,zfactj,zisw,zjsw,znot_thru_surface,zti_coord,zti_g_lim,zti_g_raw,zti_lim,zti_raw,ztj_coord,ztj_g_lim,ztj_g_raw,ztj_lim,ztj_raw)
        ! !$OMP do schedule(static)
        DO jk = 1, jpkm1
  ...
  SUBROUTINE ldf_slp_mxl(prd, pn2, p_gru, p_grv, p_dzr)
    ...
    wslpjml(1, :) = 0._wp
    wslpjml(jpi, :) = 0._wp
    !$OMP parallel default(shared), private(ik,ji,jj,jk)
    !$OMP do schedule(static)
    DO jk = 1, jpk
      DO jj = 1, jpj
        DO ji = 1, jpi
