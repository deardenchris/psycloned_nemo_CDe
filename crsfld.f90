MODULE crsfld
  ...
  SUBROUTINE crs_fld(kt)
    ...
    END DO
    !$ACC END KERNELS
    CALL profile_psy_data7 % PreStart('crs_fld', 'r7', 0, 0)
    ...
    IF (iom_use("eken")) THEN
      ...
      END DO
      !$ACC END KERNELS
      CALL profile_psy_data8 % PreStart('crs_fld', 'r8', 0, 0)
    ...
    END DO
    !$ACC END KERNELS
    CALL profile_psy_data9 % PreStart('crs_fld', 'r9', 0, 0)
    CALL crs_lbc_lnk(hdivn_crs, 'T', 1.0)
    CALL iom_put("hdiv", hdivn_crs)
    CALL profile_psy_data9 % PostEnd
    IF (ln_crs_wn) THEN
      CALL profile_psy_data10 % PreStart('crs_fld', 'r10', 0, 0)
    ...
    ELSE
      ...
      !$ACC END KERNELS
    END IF
    CALL profile_psy_data11 % PreStart('crs_fld', 'r11', 0, 0)
